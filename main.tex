...
As it was briefly mentioned before, with our Scribble extension we introduce the notion of a protocol's scope, which was not handled very explicitly in the original paper, we will proceed to describe in more detail what restrictions need to be considered when writing Scribble protocols.\\
...
The result of this renaming step is a set of protocols which do not contain any nested protocols within. We convert both nested and global protocols to this intermediate representation. However, even though this transformation allows us to treat both kinds of protocols uniformly, we will still require that the first protocol to be called is a global protocol. We define a slightly a slightly different syntax for it, which is essentially the same as a nested protocol declaration without any nested protocols inside:\\

$\protocol\ P(\role\ A_1,\ ...,\ \role\ A_n;\ \scrnew\ \role B_1,\ ...,\ \role\ B_m)\ \{\;G\;\}$\\

This renaming process requires building two environments: one to keep track of all the protocols which are in scope, which gets updated whenever new nested protocol definitions come up, and another one to keep track of all the unique protocol names which have already been used. We define an algorithm for renaming protocols in Figures \ref{renaming-interactions} and \ref{renaming-module}.
...
The algorithm described above recursively traverses all the different protocol scopes, generating new unique names for each of the protocols and updating any references to those protocols in the interactions of all the protocols for which they are in scope. 

The proposed algorithm to rename all the protocols in a module consists of 3 phases:
\begin{itemize}
    \item The first step is to update both environments with all the protocols in the top-level scope. When resolving clashes between top-level nested and global protocols we decided to always update the nested protocol name and keep the global protocol name without changing it. This approach allows us to only keep a single environment for the nested protocols which are in the current scope, so when processing calls to global protocols ($\scrdo\ P(\roles{A}{n})$) the name will remain unchanged.
\end{itemize}
...
% In the definition of projection defined above we assume that protocol names are unique, so that there will be no naming clashes when generating the local protocol signatures and there will not be any ambiguity with regards to which protocol to call. 
