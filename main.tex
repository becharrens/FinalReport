...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
We present the first implementation of the multiparty session types (MPST) theory of nested protocols, which allows calling a subprotocol during the execution of a parent protocol, possibly involving new participants in the subprotocol. We extend a MPST-based framework for specifying and statically verifying concurrent protocols with the nested protocol theory, introducing the definitions of nested protocols and nested protocol calls.\\

We design a scheme to generate APIs in Go for the roles taking part in each nested protocol, which ensures that they only perform I/O actions which comply with the protocol specification. Our implementation leverages Go's in-built concurrency primitives - \textit{goroutines} and \textit{shared memory channels} - to implement the behaviour of the roles and to allow them to communicate with each other. These primitives had not been treated in the previous MPST frameworks.\\
